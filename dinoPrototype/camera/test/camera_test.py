...
def test_4_20_checkFOV():
	...
	if plot:
		plt.figure()
		plt.imshow(noStarCam.images[0].detectorArray.reshape(
		...
		plt.figure()
		plt.imshow(noStarCam.images[1].detectorArray.reshape(
			noStarCam.resolutionWidth,
			noStarCam.resolutionHeight), cmap='Greys_r')
		plt.xlabel('Pixel')
		plt.ylabel('Line')
	...
	pdb.set_trace()
	assert(len(noStarCam.images[0].scenes[0].pixel) != 0)
	assert(len(noStarCam.images[1].scenes[0].pixel) == 0)
