...
#THIS IS WHERE I TRIED TO DO A SECONDARY FIT. 

def fcn2min(params, comp, pub):
    a = params['a']
    b = params['b']
    c = params['c']
    d = params['d']
...
minner = Minimizer(fcn2min, params, fcn_args=(computed_temps,published_temps))
result = minner.minimize()
final = computed_temps + result.residual
report_fit(result)
plt.plot(published_temps,final)
pdb.set_trace()


select_string = "SELECT VTmag, BTmag, computed_temperature from tycho_data"
c.execute(select_string)
...
for row in c:
	if row[2] != None and row[1] != '':
		...
		BTmag = row[1]
		T = row[2]
		bb_S_V_T = sum(planck(T,V_T_lambda*1e-9)*S_V_J)*dlambda_S_V_T
		bb_S_B_T = sum(planck(T,B_T_lambda*1e-9)*S_B_J)*dlambda_S_B_T
		bb_S_V_T_vega = sum(planck(T_vega,V_T_lambda*1e-9))*dlambda_S_V_T
		rsq_over_dsq = 10**(-VTmag/2.5)*(2.6496*r_sun)**2/(25.04*ly)**2*bb_S_B_T/bb_S_B_T_vega

		F_V = pi*rsq_over_dsq*planck(T,V_T_lambda*1e-9)
		F_B = pi*rsq_over_dsq*planck(T,V_T_lambda*1e-9)
...
#normalize BB for plotting
bb = bb/max(bb)

plt.plot(lambda_set_nm*1e9,bb,color='black',label='Normalized Vega Blackbody')
plt.plot(V_T_lambda,S_V_T,'g',label='Visible')
plt.plot(B_T_lambda,S_B_T,'b',label='Blue')
plt.xlabel('Wavelength (nm)')
plt.ylabel('Percent Throughput')
plt.title('Tycho Sensitivity Curves')
plt.xlim([300,800])
plt.ylim([0,1.1])
plt.legend()
...
plt.figure()
plt.plot(lambda_set_nm*1e9,bb,color='black',label='Normalized Vega Blackbody')
plt.plot(V_J_lambda,S_V_J,'g',label='Visible')
plt.plot(B_J_lambda,S_B_J,'b',label='Blue')
plt.xlabel('Wavelength (nm)')
plt.ylabel('Percent Throughput')
plt.title('Johnson Sensitivity Curves')
plt.xlim([300,800])
plt.ylim([0,1.1])
plt.legend()
