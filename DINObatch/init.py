...
def main():
    ...
    SP.furnsh(DINO_kernel)
    body_int = -100#SP.spkobj(DINO_kernel)
    body_id_str = str(body_int)

    search_window = SP.utils.support_types.SPICEDOUBLE_CELL(2)
    SP.spkcov(DINO_kernel, body_int, search_window)
    list_of_events = SP.wnfetd(search_window, 0)
    ...
    start_et = SP.utc2et('23 JUL 2020 17:00:00')
    end_et   = SP.utc2et('30 JUL 2020 17:00:00')

    # Get Observation Times and Ephemerides
    true_ephem, t_span = dg.generate_data(sc_ephem_file=DINO_kernel,
                                          planet_beacons = ['mercury','earth','mars','jupiter'],
                                          beacon_ids=\
[2008014, 2054509, 2085989, 2099935, 2144411, 2242450, 2289227, 2420302, 2480936],
                                          n_observations=26,
                                          start_et=start_et,
                                          end_et=end_et)
    ...
    print 'IC', IC
    spice_derived_state = SP.spkezr(body_id_str, t_span[0], ref='J2000', abcorr='None', obs='Sun')
    print 'Spice State', spice_derived_state
    ...
    prefits     = np.absolute(extra_data['prefit residuals'])
    postfits    = np.absolute(extra_data['postfit residuals'])
    ...
    plt.savefig('State Errors and Covariance.png', dpi=300, format='png')
    ...
    plt.tight_layout()
    plt.subplots_adjust(top=.9)
    plt.savefig('Reference Trajectory Error.png', dpi=300, format='png')
    ...
    plt.tight_layout()
    plt.subplots_adjust(top=.9)
    plt.savefig('Prefit Observation Residuals.png', dpi=300, format='png')
    ...
    plt.tight_layout()
    plt.subplots_adjust(top=.9)
    plt.savefig('Postfit Observation Residuals.png', dpi=300, format='png')
    ...
    plt.savefig('Trajectories.png', dpi=300, format='png')
    ...
    plt.tight_layout()
    plt.subplots_adjust(top=.9)
    plt.savefig('Standard Deviation and Covariance.png', dpi=300, format='png')
    ...
    plt.tight_layout()
    plt.subplots_adjust(top=.9)
    plt.savefig('Observation Errors.png', dpi=300, format='png')
    ...
    plt.tight_layout()
    plt.subplots_adjust(top=.9)
    plt.savefig('State Deviation Est Truth.png', dpi=300, format='png')
    return
