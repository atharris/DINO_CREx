...
ex_image = misc.imread('stars.jpg', mode='L')
...
pixel_line_beacon_i = [(300, 400), (200, 400), (200, 250), (250, 600)]

print '\nInitial Image Resolution and Initial Estimate Location'
print ex_res, pixel_line_beacon_i

# find centroid
pixel_line_centroid, DN = imfunc.find_centroid_point_source(ex_image, pixel_line_beacon_i, ROI_parameters, 4)
...
for i in range (0, 4):
    ...
    print '\nCentroid Location: ','%.2f' % pixel_line_centroid[i][0], '%.2f' % pixel_line_centroid[i][1]
    print 'DN Value: ', DN[i]
    ...
    if doplot == True:
        ...
        plt.imshow(ex_image)
        plt.colorbar()
        plt.scatter(pixel_line_centroid[i][1], pixel_line_centroid[i][0], color='w', marker='x', s=50)
        plt.savefig('saved_output/psf_centroid_landscape_' + str(i) + '.png')
