...
file_in = np.load('test_cases/90_deg.npz')
...
if numBeaconsResolved > 0:
    pixel_line_center, DN = imfunc.findCenterResolvedBody(ex_image, pixel_line_resolved_i, ROI_parameters)
else:
    pixel_line_center, DN = imfunc.findCentroidPointSource(ex_image, pixel_line_ptSource_i, ROI_parameters)
